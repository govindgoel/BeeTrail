import {
  ...
  const SelectCrops = ({setshowcropselection, farmer,farmdetails,setfarmdetails, selectedCrop2,setSelectedCrop2}) => {
    ...
    useEffect(() => {
      if(farmer){
        console.log(farmdetails?.bloomingcrop,'cvj ');
        setSelectedCrop(farmdetails?.bloomingcrop!==undefined?farmdetails?.bloomingcrop:[])
      }
      else{
     setSelectedCrop(selectedCrop2)
      }
    }, [])
    ...
    const handlecropcategory=(index)=>{
      let filteredData = [];
      ...
      // console.log(filteredData,activeTab);
    ...
    const handleSearch = () => {
      ...
      console.log(filteredCropData);
      setFilteredCropData(filteredData);
    };
    const checked = name => {
      let itemExists = [];
      if (selectedCrop?.length > 0)
        itemExists = selectedCrop.filter(item => item.name === name);
    ...
    const selectTab = index => {
       setActiveTab(index);
      handlecropcategory(index);
    };
  
  
    const renderCropItem = ({item,id}) => (
      <TouchableOpacity
        style={[
          styles.cropCard,
          {
            backgroundColor: checked(item.name)
              ? udyamitaTheme.beeAppColor
              : '#fff',
          },
        ]}
        onPress={() => handleItem(item.name, item)}>
        ...
        <RadioForm formHorizontal={true}>
          <RadioButton labelHorizontal={true} key={item.id}>
            {/* <RadioButtonInput
              obj={{value: item.id}}
              index={item.id}
              isSelected={checked(item.name)}
              onPress={() => handleItem(item.id, item)}
              borderWidth={checked(item.name) ? 3 : 1}
              buttonInnerColor={
                checked(item.name) ? udyamitaTheme.beeAppColor : '#e74c3c'
              }
              buttonOuterColor={
                checked(item.name) ? '#fff' : udyamitaTheme.borderStyleColor
              }
              buttonSize={9}
              buttonOuterSize={20}
            /> */}
            ...
            <RadioButtonLabel
              ...
              onPress={() => handleItem(item.name, item)}
              labelStyle={{
                color: checked(item.name) ? '#fff' : '#000',
      ...
      return (
        <>
          <View
            ...
            <Image source={item.source} style={styles.cropImage2} />
            <CustomText style={styles.cropNameSelected2} type="mlabel">
              {item.name}
            </CustomText>
            <TouchableOpacity  onPress={() => handleItem(item.name, item)}
            activeOpacity={0.6}> 
            <Image
    ...
    const handleConfirmCrops = () => {
      if(farmer){
        setfarmdetails({...farmdetails,bloomingcrop:selectedCrop})
      }
      else{
      setSelectedCrop2(selectedCrop);
      }
    ...
    const handleItem = (name, newItem) => {
      console.log(newItem, selectedCrop, 'sad');
      setSelectedCrop(prevItems => {
        // Check if the item with the given id is already in the state
        let itemExists = [];
        if (prevItems?.length > 0)
          itemExists = prevItems.filter(item => item.name === name);
    ...
    return (
      <View style={[styles.mainConatiner, {backgroundColor: '#FFFFFF'}]}>
        <View style={styles.header}>
          <View
            ...
            <TouchableOpacity onPress={() => setshowcropselection(false)}>
              <Image
        ...
        {renderTabs()}
          <FlatList
            data={searchText.length > 0 ? filteredCropData : cropData}
            renderItem={renderCropItem2}
            keyExtractor={(item,id) => id}
            numColumns={3}
            contentContainerStyle={{
              paddingBottom: selectedCrop?.length > 0 ? 500 : 420,
              justifyContent:'center',
              alignContent:'center'
             }}
        ...
        <View
          ...
          {selectedCrop?.length == 0 ? (
            <CustomText style={styles.cropLengthText} type="h">
            ...
            <>
              {
                <>
                  <CustomText style={styles.cropNameSelected} type="h">
                    Flower Selected
                  </CustomText>
                  <FlatList
                    data={selectedCrop}
                    ...
                    horizontal={true} // Set this prop to true to make the list horizontal
